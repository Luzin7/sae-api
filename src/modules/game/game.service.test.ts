...
describe('GameService.getMine', () => {
  it('passes limit and offset to repo and merges mastered games', async () => {
    const masteredGame = { id: 'g-mastered', name: 'Mastered' };
    const joinedGame = { id: 'g-joined', name: 'Joined' };
    mockRepo.findByMasterId.mockResolvedValue([masteredGame]);
    mockRepo.findByPlayerId.mockResolvedValue([joinedGame]);

    const result = await service.getMine('p1', { limit: 10, offset: 5 });

    expect(mockRepo.findByMasterId).toHaveBeenCalledWith('p1');
    expect(mockRepo.findByPlayerId).toHaveBeenCalledWith('p1', { limit: 10, offset: 5 });
    expect(result).toHaveLength(2);
    expect(result.map((g) => g.id)).toContain('g-mastered');
    expect(result.map((g) => g.id)).toContain('g-joined');
  });

  it('deduplicates games that appear in both mastered and joined', async () => {
    const game = { id: 'g1', name: 'Same Game' };
    mockRepo.findByMasterId.mockResolvedValue([game]);
    mockRepo.findByPlayerId.mockResolvedValue([game]);

    const result = await service.getMine('p1');

    expect(result).toHaveLength(1);
  });

  it('uses defaults when pagination not provided', async () => {
    mockRepo.findByMasterId.mockResolvedValue([]);
