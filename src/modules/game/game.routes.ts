...
function buildGameService(): GameService {
  return new GameService(
    {
      ...
      findByInviteCode: async (code) =>
        (await db.query.game.findFirst({ where: eq(game.inviteCode, code) })) ?? null,
      findByMasterId: (masterId) =>
        db.select().from(game).where(eq(game.masterId, masterId)),
