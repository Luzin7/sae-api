...
function buildAuthService(app) {
    return new AuthService({
        findByName: async (name) => (await db.query.player.findFirst({ where: eq(player.name, name) })) ?? null,
        create: async (data) => {
        ...
        findByToken: async (token) => (await db.query.refreshToken.findFirst({
    ...
    }, { hashPassword, verifyPassword }, { sign: (payload) => app.jwt.sign(payload) });
}
